...
test('interactive panel sends clicks/text to the real robot session and captures its PDF',async()=>{
 ...
 try{
   ...
   const context=await robot.newContext();await context.route('https://servicos.receitafederal.gov.br/**',route=>route.fulfill({contentType:'text/html; charset=utf-8',body:`<input name="niContribuinte"><input id="captcha"><button id="download" onclick="window.downloadAttempt=true;const a=document.createElement('a');a.href=window.URL.createObjectURL(new Blob([Uint8Array.from(atob('${pdf.toString('base64')}'),c=>c.charCodeAt(0))],{type:'application/pdf'}));a.download='teste.pdf';a.click();">Receber PDF de teste</button>`}));
   application.jobs.openBrowser=async()=>({browser:robot,context,busterAvailable:false,browserNote:'Fixture de teste.'});
