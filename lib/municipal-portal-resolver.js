  ...
  async resolve(company,{force=false}={}){
    ...
    for(const home of official){
      try{
        ...
        if(!rootText.includes(normalize(company.municipio)))continue;
        const queue=[root,{url:new URL('/servicos.php',root.url).href,html:null}],visited=new Set();
        while(queue.length&&visited.size<8){
          ...
          for(const link of links){
            ...
            if(link.score>=10){
              ...
              const record={codigo_ibge:company.municipio_codigo_ibge,municipio:company.municipio,uf:company.uf,prefeitura_url:root.url,certidao_url:service.url,provider,adapter:provider==='unsupported'?'generic':'provider',status:provider==='unsupported'?'PROVIDER_NAO_SUPORTADO':'PORTAL_ENCONTRADO',ultima_validacao:new Date().toISOString(),fonte:page.url,confianca:'link_oficial'};
              data.portals[key]=record;this.write(data);return record;
            }
            if(target.hostname===new URL(root.url).hostname&&!visited.has(target.href))queue.push({url:target.href,html:null});
          }
        }
      }catch{}
    }
    const result={codigo_ibge:company.municipio_codigo_ibge,municipio:company.municipio,uf:company.uf,status:'PORTAL_NAO_CADASTRADO',ultima_validacao:new Date().toISOString(),confianca:'nenhuma'};data.portals[key]=result;this.write(data);return result;
  }
}
