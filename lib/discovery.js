...
async function resolveMunicipalPortal(company,store){
  ...
  const portal={municipalityCode:company.municipalityCode,url,verifyUrl:null,sourceUrl,discoverySource:candidate.sourceUrl,coverage,mode:'assistido',allowedHosts:[...new Set([home.hostname,new URL(url).hostname,...[...visited].map(u=>new URL(u).hostname)])],registeredAt:new Date().toISOString(),registeredBy:'descoberta_por_ibge',review:'descoberto_nao_homologado'};
