...
const { MunicipalPortalResolver } = require('./municipal-portal-resolver');
...
function createApp(options = {}) {
  ...
  const municipalResolver = options.municipalResolver || new MunicipalPortalResolver({ cacheFile: path.join(store.dir || process.env.DATA_DIR || path.join(__dirname, '..', 'data'), 'municipal-portals.json') });
  const resolveMunicipalPortal = async (c) => {
    const result = await municipalResolver.resolve({ municipio_codigo_ibge:c.municipalityCode, municipio:c.municipality, uf:c.uf, cnpj:c.cnpj }, { force:true });
    if (result.status === 'PORTAL_ENCONTRADO') {
      const portal = { municipalityCode:c.municipalityCode, url:result.certidao_url, verifyUrl:result.verifyUrl || null, sourceUrl:result.fonte || result.prefeitura_url, allowedHosts:[...new Set([new URL(result.certidao_url).hostname, new URL(result.fonte || result.prefeitura_url).hostname])], provider:result.provider, coverage:'portal_identificado', mode:'assistido', registeredAt:new Date().toISOString(), registeredBy:'descoberta_automatica', review:'descoberto_nao_homologado' };
      store.put('portal', c.municipalityCode, portal); store.audit({type:'municipal_portal_discovered',cnpj:c.cnpj,portal});
  ...
  app.post('/api/companies/lookup', asyncRoute(async (req,res) => { const c = await (options.lookupCompany || lookupCompany)(req.body.cnpj,store); let discoveryError; if(routesFor(c,store).some(p=>p.coverage==='nao_cadastrado'))try{await resolveMunicipalPortal(c);}catch(e){discoveryError=e.message;} res.json({...detail(c.cnpj),discoveryError}); }));
  app.get('/api/companies/:cnpj', (req,res,next) => { try { res.json(detail(req.params.cnpj)); } catch(e) { next(e); } });
  app.post('/api/companies/:cnpj/discover', asyncRoute(async (req,res) => { const c = company(req.params.cnpj); res.json(await resolveMunicipalPortal(c)); }));
  ...
  app.post('/api/companies/:cnpj/jobs', asyncRoute(async (req,res) => {
    ...
    if(portal?.key==='municipal'&&['nao_cadastrado','diretorio_descoberto','diretorio_oficial'].includes(portal.coverage)){await resolveMunicipalPortal(c);portal=routesFor(c,store).find(p=>p.key==='municipal');} if (!portal) return res.status(400).json({error:'Portal inválido.'});
